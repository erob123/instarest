...
import uuid
from examples.example_app import crud_test, EmptyTestModel, crud_schemas
from fastapi.encoders import jsonable_encoder

EmptyTestCreate = crud_schemas.EntityCreate
...
def test_get_obj(db: Session) -> None:
    ...
    obj = crud_test.create(db=db, obj_in=obj_in)
    stored_obj = crud_test.get(db=db, id=obj.id)
    assert stored_obj
...
def test_get_obj_bad_id(db: Session) -> None:
    stored_obj = crud_test.get(db=db, id=uuid.uuid4())
...
def test_get_multi(db: Session) -> None:
    ...
    obj = crud_test.create(db=db, obj_in=obj_in)
    obj_2 = crud_test.create(db=db, obj_in=obj_in_2)
    all_objs = crud_test.get_multi(db=db, skip=0, limit=10000)
...
def test_update_obj(db: Session) -> None:
    ...
    crud_test.update(db=db, db_obj=obj, obj_in=obj_update)

    # get the obj again
    obj2 = crud_test.get(db=db, id=obj.id)
...
def test_update_obj_refreshes_if_expired(db: Session) -> None:
    ...
    crud_test.create(db=db, obj_in=obj_in_2)
    ...
    # get the obj again
    new_obj = crud_test.get(db=db, id=obj.id)
...
def test_delete_obj(db: Session) -> None:
    ...
    obj = crud_test.create(db=db, obj_in=obj_in)
    obj2 = crud_test.remove(db=db, id=obj.id)
    obj3 = crud_test.get(db=db, id=obj.id)
...
def test_delete_obj_bad_id(db: Session) -> None:
    obj = crud_test.remove(db=db, id=uuid.uuid4())
    assert obj is None
...
def test_refresh_all_by_id(db: Session) -> None:
    ...
    obj = crud_test.create(db=db, obj_in=obj_in_1)
    obj2 = crud_test.create(db=db, obj_in=obj_in_2)
    obj3 = crud_test.create(db=db, obj_in=obj_in_3)
    ...
    crud_test.update(db=db, db_obj=obj2, obj_in=obj2_update)
    assert "bool_field" in jsonable_encoder(obj2)

    # update and assert that fields were refreshed appropriately but obj2 expired
    crud_test.update(db=db, db_obj=obj3, obj_in=obj3_update)
    assert not jsonable_encoder(obj2)
    assert "bool_field" in jsonable_encoder(obj3)

    # refresh all objects
    objs = crud_test.refresh_all_by_id(db=db, db_obj_ids=[obj.id, obj2.id, obj3.id])
...
def test_refresh_all_by_id_empty_or_none_id_list(db: Session) -> None:
    ...
    objs = crud_test.refresh_all_by_id(db=db, db_obj_ids=[])
    assert objs == []

    # refresh all objects with None
    objs = crud_test.refresh_all_by_id(db=db, db_obj_ids=None)
    assert objs == []
...
def test_create_all_using_id(db: Session) -> None:
    obj_in_list = [
        EmptyTestCreate(title="1st"),
        EmptyTestCreate(title="2nd"),
        EmptyTestCreate(title="3rd"),
    ]
    objs = crud_test.create_all_using_id(db=db, obj_in_list=obj_in_list)
...
def test_create_all_using_id_empty_or_none_list(db: Session) -> None:
    ...
    objs = crud_test.create_all_using_id(db=db, obj_in_list=[])
    assert objs == []

    # create all objects with None
    objs = crud_test.create_all_using_id(db=db, obj_in_list=None)
    assert objs == []
